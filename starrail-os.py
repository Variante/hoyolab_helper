...
class Roles(Base):
    def get_awards(self):
        response = {}
        try:
            response = req.to_python(req.request('get', CONFIG.HSR_REWARD_URL, headers=self.get_header()).text)
    ...
    def run(self):
        ...
        if info_list:
            ...
            if info_list.get('data',{}).get('is_sign') is True:
                ...
                message['status'] = f"👀 Trailblazer, you've already checked in today"
            ...
            data = {
                'act_id': CONFIG.HSR_ACT_ID
            }

            try:
                response = req.to_python(req.request(
                    'post', CONFIG.HSR_SIGN_URL, headers=self.get_header(),
...
if __name__ == '__main__':
    log.info(f'🌀Honkai Star Rail Helper v{CONFIG.GIH_VERSION}')
    ...
    :param HSR_COOKIE: 米游社国际版的COOKIE.多个账号的COOKIE值之间用 # 号隔开,例如: 1#2#3#4
    """
    # Github Actions用户请到Repo的Settings->Secrets里设置变量,变量名字必须与上述参数变量名字完全一致,否则无效!!!
    # Name=<变量名字>,Value=<获取的值>
    HSR_COOKIE = ''

    if os.environ.get('HSR_COOKIE', '') != '':
        HSR_COOKIE = os.environ['OS_COOKIE']

    cookie_list = HSR_COOKIE.split('#')
